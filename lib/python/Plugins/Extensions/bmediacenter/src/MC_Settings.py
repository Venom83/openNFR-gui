...
#			evfd.getInstance().vfd_write_string(_("Settings"))
	...
	def checkNetworkStateFinished(self, result, retval,extra_args=None):
		if result.find('bad address') != -1:
			self.session.openWithCallback(self.InstallPackageFailed, MessageBox, _("Your Golden Interstar Xpeed LX is not connected to the internet, please check your network settings and try again."), type=MessageBox.TYPE_INFO, timeout=10, close_on_any_key=True)
			...
			self.session.openWithCallback(self.InstallPackage,MessageBox,_('Your Golden Interstar Xpeed LX will be restarted after the installation of service\nReady to install %s ?') % (self.service_name), MessageBox.TYPE_YESNO)
